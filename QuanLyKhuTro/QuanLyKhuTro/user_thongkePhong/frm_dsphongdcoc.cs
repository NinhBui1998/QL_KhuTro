...
{
    public partial class frm_dsphongdcoc : Form
    {
        BLL_ThongKePhong bll_tkp = new BLL_ThongKePhong();
        BLL_DSPhong bll_dsp = new BLL_DSPhong();
        public frm_dsphongdcoc()
        {
            InitializeComponent();
        }

        private void frm_dsphongdcoc_Load(object sender, EventArgs e)
        {
            grv_phongdc.DataSource = bll_tkp.loadphongcoc();
            txt_tongphongdacoc.Text = bll_dsp.tongphongcokhachcoc();
        }
        ...
        private void simpleButton3_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string saveExcelFile = @"E:\DA_TotNghiep\GitHub\QL_KhuTro\ThongKe_cocphong.xlsx";
                string folderPath = @"E:\DA_TotNghiep\GitHub\QL_KhuTro\ThongKe_cocphong.xlsx";
                ...
                if (folderBrowser.ShowDialog() == DialogResult.OK)
                {
                    ...
                    var fileCount = (from file in Directory.EnumerateFiles(folderPath, "*.xlsx", SearchOption.AllDirectories)
                                     select file).Count();
                    var s = fileCount + 1;
                    saveExcelFile = folderPath + "\\ThongKe_cocphong" + s + "_" + a + "_" + b + "_" + c + ".xlsx";
                ...
                row1_TieuDe_ThongKeSanPham.Value2 = "Danh sách phòng đã được cọc";
                ...
                //Tạo ô tháng năm :
                Range row23_slht = ws.get_Range("C2", "C3");//Cột C dòng 2 và dòng 3
                row23_slht.Merge();
                ...
                //Tạo ô tên tầng 
                Range row23_sltoida = ws.get_Range("D2", "D3");//Cột C dòng 2 và dòng 3
                row23_sltoida.Merge();
                ...
                //Tạo ô tên loại 
                Range row23_tentang = ws.get_Range("E2", "E3");//Cột C dòng 2 và dòng 3
                row23_tentang.Merge();
                ...
                //Tạo ô tên phong
                Range row23_tenloai = ws.get_Range("F2", "F3");//Cột C dòng 2 và dòng 3
                row23_tenloai.Merge();
                ...
                //Tạo ô tiền phòng
                Range row23_makhach = ws.get_Range("G2", "G3");//Cột C dòng 2 và dòng 3
                row23_makhach.Merge();
                row23_makhach.Font.Size = fontSizeTenTruong;
                row23_makhach.Font.Name = fontName;
                row23_makhach.Cells.HorizontalAlignment = XlHAlign.xlHAlignCenter;
                row23_makhach.ColumnWidth = 20;
                row23_makhach.Value2 = "Mã khách cọc phòng";

                //Tạo ô tiền tiền điện
                Range row23_ten = ws.get_Range("H2", "H3");//Cột C dòng 2 và dòng 3
                row23_ten.Merge();
                row23_ten.Font.Size = fontSizeTenTruong;
                row23_ten.Font.Name = fontName;
                row23_ten.Cells.HorizontalAlignment = XlHAlign.xlHAlignCenter;
                row23_ten.ColumnWidth = 20;
                row23_ten.Value2 = "Tên khách";



                //Tạo ô tiền tiền wifi
                Range row23_cmnd = ws.get_Range("I2", "I3");//Cột C dòng 2 và dòng 3
                row23_cmnd.Merge();
                ...
                row23_cmnd.ColumnWidth = 20;
                row23_cmnd.Value2 = "Số chứng minh nhân dân";
                //Tạo ô tiền tiền wifi
                Range row23_sdt = ws.get_Range("J2", "J3");//Cột C dòng 2 và dòng 3
                row23_sdt.Merge();
                ...
                string tong = txt_tongphongdacoc.Text;
                for (int i = 0; i < gridView2.RowCount; i++)
                {
                    ...
                    string tenp = bll_tkp.laytenphong();
                    string slht = gridView2.GetRowCellValue(i, "SOLUONG_HT1").ToString();
                    string sltd = gridView2.GetRowCellValue(i, "SOLUONG_TD1").ToString();
                    string tentang = gridView2.GetRowCellValue(i, "TENTANG1").ToString();
                    string tenloai = gridView2.GetRowCellValue(i, "TENLOAI1").ToString();
                    string makhcp = gridView2.GetRowCellValue(i, "MAKHCP1").ToString();
                    string ten = gridView2.GetRowCellValue(i, "TEN1").ToString();
                    string cmnd = gridView2.GetRowCellValue(i, "SOCMND1").ToString();
                    string dt = gridView2.GetRowCellValue(i, "SODT1").ToString();

                    dynamic[] arr = {stt,tenp,slht,sltd,tentang,tenloai,makhcp,ten,cmnd,dt};
